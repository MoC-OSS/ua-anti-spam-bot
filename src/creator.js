const { env } = require('typed-dotenv').config();

// TODO remove this file later and from Git-history
const creatorId = env.CREATOR_ID ?? 341977297;
const creatorNick = '@dimkasmile';
const logsChat = -1001599612617;
const trainingChat = env.TRAINING_CHAT_ID ?? -1001527463076;
const privateTrainingChat = -788538459;

const swindlersRegex =
  /(?:https?:\/\/)?(privat24\.|privatpay|privatbank.my-payment|app-raiffeisen|mono-bank|login24|privat\.|privat24.ua-|privatbank.u-|privatbank.m|privatbank.a|privatpay|e-pidtrimka|perekazprivat|privatbank.|privatapp)(?!ua).+/;

module.exports = {
  logsChat,
  trainingChat,
  privateTrainingChat,
  creatorNick,
  creatorId,
  swindlersRegex,
};
